...
class OthelloVectEnv(MPVectEnv):
    def __init__(self, 
        ...
        self.states = self.states.to(dtype=torch.int8)

        self.board_size = board_size
        num_rays = (8 * (self.board_size - 2)) + 1
        self.ray_tensor = torch.zeros((num_parallel_envs, num_rays, self.board_size, self.board_size), dtype=torch.int8, device=device, requires_grad=False)
        self.reset()
